...
public class MapEditor {
	public static long SEED = System.currentTimeMillis();//种子为3生成的00是城堡
...
public static final int WIDTH = 256;
	...
	public WorldMapModel create() {
		...
		//存储256个点遍历列表生成对应大区域
		List<Point> indexs = new ArrayList<>();
		for (int i = 0; i < arr.length; i+=16) {
			for (int j = 0; j < arr[0].length; j+=16) {
				indexs.add(new Point(i,j));
			}
		}
		//初始化大区域列表
		...
		//添加22个城堡
		for (int i = 0; i < 22; i++) {
            Point g = indexs.get(random.nextInt(indexs.size()));
			buildCastle(g.x,g.y);
			indexs.remove(g);
		}
		//添加56个村庄
...
//		buildRoad(castles.get(0),castles.get(1));
	...
	private List<Point> getRoadList(Point p1, Point p2){
		...
		for(AStarNode n:list){
			array.add(new Point(x0+n.getX(),y0+n.getY()));
		}
		return array;
	}
	//连接2个城堡
	private void buildRoad(Castle castle1, Castle castle2){
		...
		p2.x += castle2.getX0();
		p2.y += castle2.getY0();
		List<Point> array = getRoadList(p1,p2);
		for(Point p:array){
